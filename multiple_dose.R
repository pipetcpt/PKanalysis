...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("VP", "Peripheral volume (VP, L)", 0.1, 2500, 5),
...
server <- function(input, output, session) {
  ...
  output$table <- renderTable({
    ...
    a <- tblNCA(p, key = "ID", colTime = "time", colConc = "CP", dose = 5, adm = "Extravascular", doseUnit = "mg", timeUnit = "h", concUnit = "ng/mL", down = "Linear", R2ADJ = 0.1)
    a[, c("LAMZHL", "LAMZ")]
  })
